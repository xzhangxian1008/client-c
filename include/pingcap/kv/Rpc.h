...
public:
    ...
    std::string errMsg(const ::grpc::Status & status, const std::string & extra_msg)
    {
        auto msg = std::string(T::errMsg()) + " " + std::to_string(status.error_code()) + ": " + status.error_message();
        if (!extra_msg.empty())
        {
            msg += " " + extra_msg;
        }
